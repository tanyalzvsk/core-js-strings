...
function isPalindrome(str) {
  ...
  const reversedStr = newStr.split('').reverse();
  const comparedStr = reversedStr.join('');
  if (newStr === comparedStr) {
    return true;
  }
  return false;
}
...
function extractNameFromTemplate(value) {
  const firstPart = value.indexOf('Hello, ') + 7;
  const lastPart = value.indexOf('!');
  return value.substring(firstPart, lastPart).trim();
}
...
function encodeToRot13(str) {
  ...
  const cheat = 'nopqrstuvwxyzabcdefghijklmNOPQRSTUVWXYZABCDEFGHIJKLM';
  let encodedStr = '';
  for (let i = 0; i < str.length; i += 1) {
    const char = str[i];
...
function getCardId(value) {
  ...
  const res = (card) => card === value;
  return cardDeck.findIndex(res);
}
